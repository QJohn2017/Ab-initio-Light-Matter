...
void Compare(util::banded<real> x, std::vector<real> y)
{
    EXPECT_EQ(x.size(), y.size());
    for (size_t i = 0; i < x.size(); i++)
        ASSERT_DOUBLE_EQ(x(i), y[i])  << "Loop: " << i << std::endl;
...
void PrintArrayAsVec(std::string Name, util::banded<real> & Array)
{
...
//     quadrature::gauss Gauss(std::vector<std::pair<real, real>> Val);

    quadrature::gauss Gauss(9);

    util::banded<real> DivX2(No, Nw), H(No, Nw);
...
//     PrintArrayAsVec("DivX2Compare", DivX2);
//     PrintArrayAsVec("HCompare", H);
...
{
    std::cout << "Gaussian Quadrature test not implemented yet.\n";
    quadrature::gauss Gauss(9);
    /*
